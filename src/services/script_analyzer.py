...
class ScriptAnalyzerService:
            ...
            if len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
                if isinstance(node.value, ast.Call):
                    ...
                    if isinstance(node.value.func, ast.Name):
                        # Direct function/class call - determine which one
                        ...
                        if self._is_likely_class_instantiation(func_name, full_name):
                            ...
                            # It's a function call assigned to a variable
                            self._extract_function_call(node.value, result)
    ...
    def _is_likely_class_instantiation(self, func_name: str, full_name: Optional[str]) -> bool:
        ...
        # Functions that start with lowercase and don't match class patterns are likely functions
