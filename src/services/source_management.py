...
class SourceManagementService:
    ...
    async def get_knowledge_base_guide(self, knowledge_graph_service=None) -> Dict[str, Any]:
        """
        Generate a knowledge base guide with available resources.

        Args:
            knowledge_graph_service: Optional knowledge graph service for repository data
        ...
        try:
            ...
            # Get knowledge graph repositories if service is available
            knowledge_graph_repos = []
            if knowledge_graph_service:
                try:
                    knowledge_graph_repos = await knowledge_graph_service.list_repositories()
                except Exception as e:
